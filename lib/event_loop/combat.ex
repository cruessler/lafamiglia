defmodule LaFamiglia.Combat do
  ...
  def calculate(%Combat{attack: attack} = combat) do
    ...
    defender = if attack.target.is_occupied, do: attack.target.occupation, else: target
  ...
  defp calculate_combat_values(result) do
    attack_value =
      Enum.reduce(Unit.all(), 0, fn u, acc ->
    ...
    defense_value =
      Enum.reduce(Building.all(), defense_value, fn b, acc ->
        acc + b.defense.(Map.get(result.defender, b.key))
      end)

    %{result | attack_value: attack_value, defense_value: defense_value}
  end
  ...
  defp calculate_losses(result) do
    attacker_losses = Unit.multiply(result.attacker_before_combat, result.attacker_percent_loss)
    defender_losses = Unit.multiply(result.defender_before_combat, result.defender_percent_loss)

    attacker_supply_loss = Unit.supply(attacker_losses)
    defender_supply_loss = Unit.supply(defender_losses)

    attacker_after_combat = Unit.subtract(result.attacker_before_combat, attacker_losses)
    defender_after_combat = Unit.subtract(result.defender_before_combat, defender_losses)
  ...
  defp calculate_results_in_occupation(result) do
    results_in_occupation = result.attacker_after_combat[@unit_for_occupation] > 0

    %{result | results_in_occupation?: results_in_occupation}
  end

  defp calculate_plundered_resources(%CombatResult{results_in_occupation?: true} = result),
