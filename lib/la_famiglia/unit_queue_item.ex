defmodule LaFamiglia.UnitQueueItem do
  ...
  def units_recruited_between(item, time_begin, time_end) do
    ...
    start_number = trunc(Timex.diff(time_begin, start_time, :microseconds) / training_time)
    end_number = trunc(Timex.diff(time_end, start_time, :microseconds) / training_time)
  ...
  def enqueue(%Changeset{data: villa} = changeset, unit, number) do
    ...
    new_item =
      Ecto.build_assoc(villa, :unit_queue_items,
        unit_id: unit.id,
        number: number,
