defmodule LaFamiglia.Villa do
  ...
  alias LaFamiglia.{AttackMovement, ComebackMovement, Occupation}
  ...
  schema "villas" do
    ...
    belongs_to :player, Player, on_replace: :nilify
    ...
    has_many :attack_movements, AttackMovement, on_replace: :delete, foreign_key: :origin_id
    has_many :comeback_movements, ComebackMovement, on_replace: :delete, foreign_key: :origin_id
  ...
  defp validate_resources(changeset) do
    enough_resources = Enum.all?(Resource.all(), fn r -> get_field(changeset, r) > 0 end)
  ...
  def empty_coordinates({x_1, x_2, y_1, y_2}) do
    ...
    villas_in_rectangle = in_rectangle(x_1, x_2, y_1, y_2) |> Repo.all() |> Enum.count()
  ...
  def add_resources(%Changeset{} = changeset, resources) do
    ...
    Enum.reduce(Resource.all(), changeset, fn r, changeset ->
      ...
      |> put_change(
        r,
        min(
          get_field(changeset, r) + Map.get(resources, r, 0),
          storage_capacity / 1
        )
      )
    end)
  ...
  def resource_gains(%Villa{} = villa, time_diff) do
    for {k, v} <- Mechanics.resource_gains(villa),
        into: %{},
