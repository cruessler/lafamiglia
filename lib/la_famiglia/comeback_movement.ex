defmodule LaFamiglia.ComebackMovement do
  ...
  schema "comeback_movements" do
    ...
    field :arrives_at, :utc_datetime_usec
  ...
  def from_combat(attack, result) do
    duration_of_return =
      duration(attack.origin, attack.target, units(result.attacker_after_combat))
  ...
  def from_attack(attack) do
    ...
    time_remaining = Timex.diff(attack.arrives_at, LaFamiglia.DateTime.now(), :microseconds)
  ...
  def from_occupation(occupation) do
    ...
    arrives_at = LaFamiglia.DateTime.from_now(microseconds: duration_of_return)
