...
class PlayerSelector extends React.Component {
  constructor(props) {
    ...
    this.bloodhound = new Bloodhound({
      ...
      remote: {
        url: '/api/v1/players/search/%QUERY',
        // For typeahead 0.10.
        ajax: {
  ...
  addPlayer(player) {
    if (!this.state.players.some(p => p.id == player.id)) {
      this.setState({ name: '', players: [...this.state.players, player] });
    }
  }
  ...
  render() {
    const playerSpans = this.state.players.map(p => {
      return (
        <span className="selected" key={p.id}>
          {p.name}
          <button
            type="button"
            className="close"
    ...
    return (
      <div className="form-control container" ref={c => (this.container = c)}>
        {playerSpans}
        <input
          type="text"
          ref={i => (this.input = i)}
