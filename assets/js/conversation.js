...
class Conversation extends React.Component {
  ...
  headerNodeForMessage(current, previous) {
    if (previous == undefined || current.sender.name != previous.sender.name) {
      return (
        <li
          className={this.classNameForMessageHeader(current)}
  ...
  messageNode(message) {
    return (
      <li key={message.id} className={this.classNameForMessage(message)}>
        {message.text}
      </li>
    );
  }
  ...
  render() {
    const [liNodes, _] = this.props.messages.reduce(
